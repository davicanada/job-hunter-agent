...
`src/db/client.py` is the single entry point for all Supabase I/O — models in / models out (Pydantic v2 in `src/models/job.py`). Schema lives in `src/db/schema.sql`; incremental migrations in `src/db/migrations/NNN_*.sql` (apply via Supabase SQL editor or Supabase MCP). Current migrations: `002_skipped_jobs`, `003_llm_calls`, `004_recency`, `005_notified_at`, `006_target_region` (renames `jobs.allows_canada` → `jobs.allows_target_region` for the broadened Canada/USA/Europe scope), `007_brazil_backfill` (re-opens historical LATAM/Brazil rows after the 2026-05-11 broadening that added Brazil to the target region).
...
- **Prefilter is strict (junior-only).** Hard-rejects any title containing senior / sr / lead / manager / mid-level / intermediate, plus the original staff / principal / director / VP / head-of / chief, plus 7+ years in title, domain blocklist (nursing, sales, customer success, business development, medical coding, performance marketing, crypto trading, L&D, etc.), stack mismatches (iOS / Unity / firmware / mechanical / hardware), and `allows_target_region=False`. The `seniority_hint` soft-flag path from Block 3.5 is gone — `_HARD_TITLE_RE` uses word boundaries so "seniority" / "leadership" / "managerial" are safe false-positive cases. When **loosening** prefilter rules that already dropped rows, run `python -m src.main rescore` to re-evaluate the `skipped_jobs` backlog.
- **Region scope (Canada + USA + Europe + Brazil).** `is_in_target_region()` (in `src/sources/base.py`) is the single source of truth. `TARGET_REGION_KEYWORDS` / `TARGET_REGION_BLOCKED_KEYWORDS` live in `config/sources.py`. Postings limited to a single in-scope region (US-only, EU-only, Brazil-only) are accepted at the source layer; the LLM scorer judges whether work-authorization actually fits and sets `auth_status` accordingly. Davi is a Brazilian citizen, so LATAM / Latin-America / South-America postings are accepted (Brazil ⊂ LATAM) and Brazil-only postings map to `auth_status="ok_work_permit"`. The blocked list only catches postings restricted to regions still outside scope (APAC, India-only, Mexico-only, etc.).
