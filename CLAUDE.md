...
- **Region scope (Canada + USA + Europe).** `is_in_target_region()` (in `src/sources/base.py`) is the single source of truth. `TARGET_REGION_KEYWORDS` / `TARGET_REGION_BLOCKED_KEYWORDS` live in `config/sources.py`. Postings limited to a single in-scope region (US-only, EU-only) are accepted at the source layer; the LLM scorer judges whether work-authorization actually fits and sets `auth_status` accordingly. The blocked list only catches postings restricted to regions outside scope (APAC, India, LATAM, etc.).
